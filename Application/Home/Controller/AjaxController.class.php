...
//			array_multisort($tmp,SORT_ASC,$marketsy);
			foreach ($marketsy as $k => $v) {
				...
				$data[$k]['name'] = str_replace("cny","cnyt",$v['name']);
				$data[$k]['img'] = $v['xnbimg'];
				$data[$k]['title'] = str_replace("CNY","CNYT",$v['title']);
                $data[$k]['price'] = $v['new_price'];
                if($v['change']>0){
	...
	public function getJsonMenu_index($ajax = 'json')
	{
		$data = (APP_DEBUG ? null : S('getJsonMenu_index'));
		if (!$data) {
...
/* 				if (C('market')[$v['name'] . '_cny']['new_price']) {
	...
	public function allcoin_a($id=1,$ajax = 'json')
	{
		...
		$allcoin_a1 = file_get_contents("allcoin_a1.txt");
		if($allcoin_a1 > time()-1800){
			$allcoin_a = file_get_contents("allcoin_a.txt");
			if($allcoin_a){
				echo $allcoin_a;
				exit;
			}
...
/*         if ($marketLogs) {
		...
		if (!$data) {
			...
			$myfile = fopen("allcoin_a.txt", "w");
			fwrite($myfile, json_encode($qq3479015851_data));
			fclose($myfile);
			$myfile = fopen("allcoin_a1.txt", "w");
			fwrite($myfile, time());
			fclose($myfile);
		if ($ajax) {
			echo json_encode($qq3479015851_data);
		...
		if (!$data) {
			if ($market) {
				...
				foreach (C('market') as $k => $v) {
					$v['xnb'] = explode('_', $v['name'])[0];
					$v['rmb'] = explode('_', $v['name'])[1];
					$data['list'][$k]['name'] = str_replace("cny","cnyt",$v['name']);
	...
	public function getJsonUserCoin($ajax = 'json'){
    ...
    public function getEntrustFinish($market = NULL,$ajax = 'json'){
        if (!userid()) {
            return false;
        }
        ...
        $where['market'] = $market;
        $status = 2;
        $where['status'] = $status - 1;//查询已完成
        $where['userid'] = userid();
        $Moble = M('Trade');
        $count = $Moble->where($where)->count();
        $Page = new \Think\Page($count, 15);
        $Page->parameter .= '&status=' . $status . '&market=' . $market . '&';
        $show = $Page->show();
        $list = $Moble->where($where)->order('id desc')->select();
        foreach ($list as $k => $v) {
