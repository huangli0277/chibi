...
//eth 357898638
    public function qianbaoethb8c3b3d94512472db8_1($b = NULL)
    {
        ...
        foreach ($coinList as $k => $v) {
            ...
            if ($b) {
                ...
                $listtransactions = $EthPayLocal->EthInCrontab($v['block']);
                if ($listtransactions[0]['block']) {
                    M()->execute("UPDATE `qq3479015851_coin` SET  `block` =  '" . $listtransactions[0]['block'] . "' ");
                }
            }
            echo $listtransactions[0]['block'] . '==>' . $listtransactions[0]['num'] . '==>';
            //echo json_encode($listtransactions);
            echo 'start ' . $coin . "=>" . count($listtransactions) . "<br>";
            foreach ($listtransactions as $ks => $trans) {
                $coinb = $coin . "b";
                if (!$trans['to']) {
                    //echo $ks.'empty to continue' . "<br>";
                    continue;
                }
                if ($trans['number'] <= 0) {
                    //echo $ks.'==>empty account continue' . "<br>";
                    if ($token) {
                        //查询合约
                        $token = strtolower($token);
                        if ($trans['to'] != $token) continue;
                        if (!$trans['input']) continue;
                        $tos = substr($trans['input'], 34, 40);
                        if (!$tos) continue;
                        $tos = "0x" . $tos;
                        $user = M('UserCoin')->where(array($coinb => $tos))->select();
                        if (!$user) continue;
                        $user = $user[0];
                        if ($user) {
                            $to_data = substr($trans['input'], 74);
                            $num = $EthCommon->fromWei($to_data);
                            if ($num <= 0) continue;
                            if (M('Myzr')->where(array('txid' => $trans['hash']))->find()) {
                                echo $to_data . '=>' . $num . '=>hash had found continue' . "<br>";
                                continue;
                            }
                            echo $ks . 'all check ok ' . "<br>";
                            $sfee = 0;
                            $true_amount = $num;
                            if (C('coin')[$coin]['zr_zs']) {
                                $song = round(($num / 100) * C('coin')[$coin]['zr_zs'], 8);
                                if ($song) {
                                    $sfee = $song;
                                    $num = $num + $song;
                                }
                            }
                            M('myzr')->add(array('userid' => $user['userid'], 'username' => $tos, 'coinname' => $coin, 'fee' => $sfee, 'txid' => $trans['hash'], 'num' => $true_amount, 'mum' => $num, 'addtime' => time(), 'status' => 1));

                            $mo = M();
                            //$rs[] = $mo->table('qq3479015851_user_coin')->where(array('userid' => $user['userid']))->setInc($coin, $num);
                            //$mo->execute('commit');
                            echo $num . ' receive ok ' . $coin;
                            //$mo->execute('unlock tables');
                            echo $ks . 'commit ok' . "<br>";
                ...
                $user = M('UserCoin')->where(array($coinb => $trans['to']))->select();
                if (!$user) {
                    //echo $ks.'no account find continue' . "<br>";
                    continue;
                }
                $user = $user[0];
                ...
                if (M('Myzr')->where(array('txid' => $trans['hash']))->find()) {
                    echo $trans['to'] . '=>' . $trans['number'] . '=>hash had found continue' . "<br>";
                    continue;
                }

                echo $ks . 'all check ok ' . "<br>";

                $true_amount = $trans['number'];
                $sfee = 0;
                if (C('coin')[$coin]['zr_zs']) {
                    $song = round(($trans['number'] / 100) * C('coin')[$coin]['zr_zs'], 8);

                    if ($song) {
                        $sfee = $song;
                        $trans['number'] = $trans['number'] + $song;
                    }
                }
                M('myzr')->add(array('userid' => $user['userid'], 'username' => $trans['to'], 'coinname' => $coin, 'fee' => $sfee, 'txid' => $trans['hash'], 'num' => $true_amount, 'mum' => $trans['number'], 'addtime' => time(), 'status' => 1));

                $mo = M();
                $rs[] = $mo->table('qq3479015851_user_coin')->where(array('userid' => $user['userid']))->setInc($coin, $trans['number']);
                //$mo->execute('commit');
                echo $trans['number'] . ' receive ok ' . $coin . ' ' . $trans['number'];
                //$mo->execute('unlock tables');
                echo $ks . 'commit ok' . "<br>";
            }
...
//eth 357898638
    public function qianbaoethb8c3b3d94512472db8($b = 0)
    {
        $coinList = M('Coin')->where(array('status' => 1, 'type' => 'eth'))->select();
        foreach ($coinList as $k => $v) {
            if (strlen($v['dj_yh']) == 42) {
                $cos[$v['name']] = $v['dj_yh'];
            }
            if ($v['block'] > $cos['block']) $cos['block'] = $v['block'];
        }
        //print_r($cos);
        $EthCommon = new \Org\Util\EthCommon(COIN_ADDR, COIN_PORT, "2.0");
        $EthPayLocal = new \Org\Util\EthPayLocal(COIN_ADDR, COIN_PORT, "2.0", COIN_CAIWU);
        if ($b) {
            ...
            $listtransactions = $EthPayLocal->EthInCrontab($cos['block']);
            if ($listtransactions[0]['block']) M()->execute("UPDATE `qq3479015851_coin` SET  `block` =  '" . $listtransactions[0]['block'] . "' ");
        }
        echo $listtransactions[0]['block'] . '==>' . $listtransactions[0]['num'] . '==>';
        foreach ($listtransactions as $ks => $trans) {
            if (!$trans['to']) continue;//没有收款人
            if ($trans['number'] <= 0) {
                //合约
                foreach ($cos as $kc => $vc) {
                    if ($kc == "block") continue;
                    if (!$trans['input']) continue;
                    $coin = $kc;
                    $coinb = $kc . "b";
                    $token = strtolower($vc);
                    //if($token=="0x4f878c0852722b0976a955d68b376e4cd4ae99e5") continue;
                    if ($trans['to'] != $token) continue;
                    $tos = substr($trans['input'], 34, 40);//收款人
                    if (!$tos) continue;
                    $tos = "0x" . $tos;
                    $user = M('UserCoin')->where(array($coinb => $tos))->find();
                    if (!$user) continue;
                    ...
                    if (M('Myzr')->where(array('txid' => $trans['hash']))->find()) {
                        //已经入账
                        echo $tos . '=>' . $num . '=>hash had found continue' . "<br>";
                        continue;
                    }
                    echo $coin . ' all check ok ' . "<br>";
                    M('myzr')->add(array('userid' => $user['userid'], 'username' => $tos, 'coinname' => $coin, 'fee' => 0, 'txid' => $trans['hash'], 'num' => $num, 'mum' => $num, 'addtime' => time(), 'status' => 1));
                    M()->table('qq3479015851_user_coin')->where(array('userid' => $user['userid']))->setInc($coin, $num);
                ...
                M('myzr')->add(array('userid' => $user['userid'], 'username' => $trans['to'], 'coinname' => 'eth', 'fee' => 0, 'txid' => $trans['hash'], 'num' => $trans['number'], 'mum' => $trans['number'], 'addtime' => time(), 'status' => 1));
                $rs[] = M()->table('qq3479015851_user_coin')->where(array('userid' => $user['userid']))->setInc("eth", $trans['number']);
...
//eth 357898638
    ...
    public function upTrade_qq34790158511111111_8a201aa602cd9448($market = NULL)
    {
        die();
        $userid = rand(1, 2);
        $type = rand(1, 2);
