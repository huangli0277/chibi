...
class TradeModel extends \Think\Model
{
	...
	public function chexiao($id = NULL)
	{
		...
		if ($trade['type'] == 1) 
		{
			...
			$rs[] = $mo->execute("update qq3479015851_user_coin set cny = cny + {$save_buy_rmb}, cnyd = cnyd - {$save_buy_rmb}  where userid = {$trade["userid"]} limit 1");
			...
			$s_file = 'dj.txt';
			$person = "\n\r" . $rs['0'] . '------' . $save_buy_rmb . '-----' . date("H:i:s");
			file_put_contents($s_file, $person, FILE_APPEND);
