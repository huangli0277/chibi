...
function tradenoa()
{
    return substr(str_shuffle('ABCDEFGHIJKLMNPQRSTUVWXYZ'), 0, 7);
}
...
function SendEmail($data)
{
    ...
    curl_setopt($ci, CURLOPT_POSTFIELDS, json_encode($data, JSON_UNESCAPED_UNICODE));
 ...
 * cur 当前操作
 */
function SendText($cell, $code, $type)
{
    $sign = "bjs";
    ...
    switch ($type) {
        case "reg":
            ...
            break;
        case "chg":
            if ($dest == "china") $template = "SMS_142146990";
            else $template = "SMS_142146995";
            break;
        case "fog":
            if ($dest == "china") $template = "SMS_142146977";
            else $template = "SMS_142146979";
            break;
        case "cur":
            if ($dest == "china") $template = "SMS_142152010";
...
function send_moble($moble, $content)
{
    ...
    $smsapi = "http://apis.laidx.com/SMS/Send";
    $user = '0A2DC404B06D4AA0AB8CB8FBA8D99448'; //短信平台帐号
    $pass = 'dfd801e71aac4b8fb916b00f83eac83d'; //短信平台密码
    $time = $_SESSION['time'];
    ...
    $sendurl = $smsapi . "?account=" . $user . "&token=" . $pass . "&mobile=" . $moble . "&content=" . urlencode($content) . "&type=0";
...
{
    $_cell_head = substr($cell, 0, 3);
    $_cell_tail = substr($cell, 7, 4);
    $_cell = $_cell_head . "****" . $_cell_tail;
    return $_cell;
}

function hideIdCardNumber($id)
{
    $_id_head = substr($id, 0, 3);
    $_id_tail = substr($id, 14, 4);
    $_id = $_id_head . "***********" . $_id_tail;
    return $_id;
}
